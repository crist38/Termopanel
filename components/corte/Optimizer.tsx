...
import { Plus, Trash2, Box, RefreshCw, Monitor, Layers, ChevronLeft, ChevronRight, Download, PackageOpen, X, Check } from "lucide-react";
import { cn } from "@/lib/utils";
import { Item, SheetResult, packMultipleSheets } from "@/lib/packer";
import jsPDF from "jspdf";
import { CorteImport, GrupoCorte, CORTE_IMPORT_KEY } from "@/lib/types/corte";
...
function ImportPanel({
    ...
    corteImport: CorteImport;
    onImport: (grupo: GrupoCorte) => void;
    onDismiss: () => void;
}) {
    const [selected, setSelected] = useState<string | null>(null);
    const grupo = corteImport.grupos.find(g => g.key === selected);

    return (
        <div className="mb-8 bg-cyan-950/80 border border-cyan-700 rounded-2xl p-5 shadow-xl animate-in slide-in-from-top-4 duration-400">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-white font-bold text-base">
                            Importar desde Cotizaci&oacute;n
                        </h3>
                        <p className="text-cyan-300 text-xs mt-0.5">
                            {corteImport.clientName}{corteImport.obra ? ` · ${corteImport.obra}` : ''}
                        </p>
                    </div>
                </div>
                <button
            ...
            <p className="text-slate-300 text-xs mb-3">Selecciona el grupo de cristales que quieres optimizar:</p>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mb-4">
                {corteImport.grupos.map(g => (
                    <button
                        key={g.key}
                        ...
                        className={cn(
                            ...
                            selected === g.key
                                ? "bg-cyan-500/25 border-cyan-400 shadow-md shadow-cyan-900/50"
                                : "bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20"
                        )}
                    >
                        <div className="flex items-center gap-2 w-full">
                            {selected === g.key && <Check className="w-3.5 h-3.5 text-cyan-400 shrink-0" />}
                            <span className="font-semibold text-white text-sm">
                                {g.tipo} {g.espesor}mm
                            </span>
                            <span className="ml-auto text-[10px] text-cyan-400 font-mono bg-cyan-500/10 px-1.5 py-0.5 rounded">
                                C{g.cristalNum}
                            </span>
                        </div>
                        <span className="text-slate-400 text-xs mt-1">
                            {g.piezas.length} ref&nbsp;·&nbsp;{g.totalPiezas} piezas
                        </span>
                    </button>
                ))}
            </div>
            ...
            <button
                onClick={() => grupo && onImport(grupo)}
                disabled={!selected}
                className="w-full py-2.5 px-4 bg-cyan-500 hover:bg-cyan-400 disabled:opacity-40 disabled:cursor-not-allowed text-slate-950 font-bold rounded-xl text-sm transition-colors flex items-center justify-center gap-2"
            >
                <PackageOpen className="w-4 h-4" />
                {selected ? `Importar ${grupo?.totalPiezas} piezas — ${grupo?.tipo} ${grupo?.espesor}mm` : 'Selecciona un grupo'}
            </button>
        </div>
    );
}
...
export function Optimizer() {
    ...
    // --- Import from cotizador ---
    const [corteImport, setCorteImport] = useState<CorteImport | null>(null);

    useEffect(() => {
        const raw = localStorage.getItem(CORTE_IMPORT_KEY);
        if (raw) {
            try {
                const parsed: CorteImport = JSON.parse(raw);
                if (parsed.grupos?.length) {
                    setCorteImport(parsed);
                }
            } catch { /* ignore */ }
        }
    }, []);

    const handleImportGrupo = (grupo: GrupoCorte) => {
        const newItems: Item[] = grupo.piezas.map(p => ({
        ...
        setItems(newItems);
        setCorteImport(null);
        localStorage.removeItem(CORTE_IMPORT_KEY);
    };
